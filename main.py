...
# === CONFIGURAZIONE BASE ===
TOKEN = "8349278486:AAEmkSGmlH71pbRjGlEd3sQcNwxgm5CEN90"  # ← sostituisci con il tuo vero token
SOGLIA_INIZIALE = 20
...
async def pianto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pianti == soglia:
        ...
        try:
            ...
            messaggio += f" Ha superato il limite stagionale. Sarà muto per {durata_ore} or{'a' if durata_ore == 1 else 'e'}."
        except:
...
async def main():
    ...
    await app.start()
    await asyncio.Event().wait()  # mantiene attivo il bot
